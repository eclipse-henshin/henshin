...
public class GraphItemProvider extends NamedElementItemProvider implements
	...
	public void notifyChanged(Notification notification) {
		...
		case HenshinPackage.GRAPH__EDGES:
			if (notification.getEventType() == Notification.ADD
				...
				notifyCorrespondingEdges((Graph) notification.getNotifier(),
						notification);
	...
	public void notifyCorrespondingEdges(Graph graph, Notification notification) {
		if (HenshinRuleAnalysisUtil.isLHS(graph)) {
			...
			ItemProviderAdapter adapter = (ItemProviderAdapter) this.adapterFactory
					.adapt(rhs, Graph.class);
			adapter.fireNotifyChanged(new ViewerNotification(notification, rhs,
					true, true));
		}
		if (HenshinRuleAnalysisUtil.isRHS(graph)) {
			Graph lhs = graph.getContainerRule().getLhs();
			ItemProviderAdapter adapter = (ItemProviderAdapter) this.adapterFactory
					.adapt(lhs, Graph.class);
			adapter.fireNotifyChanged(new ViewerNotification(notification, lhs,
					true, true));
		}
	}
	...
	private boolean isHostGraph(Object g) {
		return (g != null) && (g instanceof EObject)
				& (((EObject) g).eContainer() != null)
