 ...
 * Here you need to obtain the Transition model element from the ReconnectRequest,
 * create a new TransitionReconnectCommand, set the new connection <i>source</i> by calling
 * the <tt>setNewSource(State)</tt> method and return the command instance.
 * <li>Override the <tt>getReconnectTargetCommand(...)</tt> method.</li>
 * Here again you need to obtain the Transition model element from the ReconnectRequest,
...
public class TransitionReconnectCommand extends Command {
	...
	private final State oldSource;
	/** The original target endpoint. */
	private final State oldTarget;

	/**
	 * Instantiate a command that can reconnect a Transition instance to a different source
	 * or target endpoint.
	 * @param conn the connection instance to reconnect (non-null)
	 * @throws IllegalArgumentException if conn is null
	 */
	public TransitionReconnectCommand(Transition conn) {
		if (conn == null) {
	...
	private boolean checkSourceReconnection() {
		...
		for (Iterator iter = newSource.getOutgoing().iterator(); iter.hasNext();) {
	...
	private boolean checkTargetReconnection() {
		...
		// return false, if the connection exists already
		for (Iterator iter = newTarget.getIncoming().iterator(); iter.hasNext();) {
			Transition conn = (Transition) iter.next();
	 ...
	 * to the supplied State instance.
	 * <p>
	 ...
	 * endpoint.
	 * </p>
	 * @param connectionSource a non-null State instance, to be used as a new source endpoint
	 * @throws IllegalArgumentException if connectionSource is null
	 */
	public void setNewSource(State connectionSource) {
	 ...
	 * endpoint.
	 * </p>
	 * @param connectionTarget a non-null State instance, to be used as a new target endpoint
	 * @throws IllegalArgumentException if connectionTarget is null
	 */
	public void setNewTarget(State connectionTarget) {
