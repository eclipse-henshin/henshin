...
//			String ruleDetails = analyseDetailsOfRule(rule2);
	...
	public void exportStoredRuntimeToCSV(String targetFolder){
		try {
			...
			Date start = new Date();
			String filename = targetFolder + File.separator + dateFormat.format(start) + "__minimalReasons-runtime.csv";
...
//						addRowStatistic();
	...
	public void exportStoredConflictsToCSV(String targetFolder){
		try {
			...
			Date start = new Date();
			String filename = targetFolder + File.separator + dateFormat.format(start) + "__minimalReasons.csv";
