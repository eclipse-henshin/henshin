...
public class MinimalConflReasonLoggerPB extends LoggerPB{
...
//			String ruleDetails = analyseDetailsOfRule(rule2);
	...
	public void exportStoredRuntimeToCSV(String targetFolder){
		try {
			...
			Date start = new Date();
			String filename = targetFolder + File.separator + dateFormat.format(start) + "__minReas-runt.csv";
...
//						addRowStatistic();
	...
	public void exportStoredConflictsToCSV(String targetFolder){
		try {
			...
			Date start = new Date();
			String filename = targetFolder + File.separator + dateFormat.format(start) + "__minReasons.csv";
