		...
		for (NegativeApplicationCondition nac : ((GraphWithNacs) (SamGraphInvCheckGraphAdapter.getInstance(pattern))).getNacs()) {
			...
			while(sgIter.hasNext()) {
				...
				if (singleNac != null && number % 200 == 0) {
					...
					//System.out.println("matches for " + number + ": " + matches);
				}
				...
				/*if (matches >= 12000 && number >= 250) {
					for (NegativeApplicationCondition nacToRemove : newNacs) {
						//remNo++;
				...
				while (currentMatch != null/* || emptyMatch == null*/) {
					...
					if (g.getNodes().size() - interfaceNodes.size() <= gm.getNodeMatching().size() && g.getEdges().size() == gm.getEdgeMatching().size()) {						
						return null;
					}
