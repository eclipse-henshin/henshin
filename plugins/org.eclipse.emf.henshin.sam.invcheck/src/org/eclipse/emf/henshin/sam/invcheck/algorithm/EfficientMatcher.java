...
import org.eclipse.emf.henshin.sam.invcheck.filter.CombinationProducer;
		...
		for (Edge e : this.host.getEdges()) {
			...
			if (srcInHost != null && tarInHost != null) {
				...
				for (Edge currentEdge : srcInHost.getOutgoing()) {
					if (this.pattern.getEdges().contains(currentEdge) && !this.currentResult.getEdgeMatching().containsValue(matchingEdge)) {
