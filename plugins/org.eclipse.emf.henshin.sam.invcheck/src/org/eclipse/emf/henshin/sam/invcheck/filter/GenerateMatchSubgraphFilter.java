...
public class GenerateMatchSubgraphFilter
		extends
		FilterSkeleton<Pair<Graph, GraphRule>, Pair<Pair<Graph, GraphRule>, Match>> {
	...
	public void produce() {
		try {
			...
			this.defaultOutputPipe.queue(new CombinationProducer.Pair<Pair<Graph, GraphRule>, Match>(this.currentInput, SamtraceFactory.eINSTANCE.createMatch()));					
		...
		while (running && mIter.hasNext()) {
			...
			try {
				this.wroteItems++;
				this.defaultOutputPipe.queue(new CombinationProducer.Pair<Pair<Graph, GraphRule>, Match>(this.currentInput, gm.copy()));													
			} catch (InterruptedException e) {
				this.running = false;
			}
			...
			for (Edge e : this.currentInput.second.getRight().getEdges()) {
				...
				if (srcInHost != null && tarInHost != null) {
					...
					for (Edge currentEdge : srcInHost.getOutgoing()) {
						if (this.currentInput.first.getEdges().contains(currentEdge) && !edgeMatching.getEdgeMatching().containsValue(matchingEdge)) {
			...
			if (edgeMatching.getEdgeMatching().size() > 0) {
				...
				while (bs.cardinality() < edges.length && running) {
					...
					this.wroteItems++;
					try {
						this.defaultOutputPipe.queue(new CombinationProducer.Pair<Pair<Graph, GraphRule>, Match>(this.currentInput, completedMatch));
