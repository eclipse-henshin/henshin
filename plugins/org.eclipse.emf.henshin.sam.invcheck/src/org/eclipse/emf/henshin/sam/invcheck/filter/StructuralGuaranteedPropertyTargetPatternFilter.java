...
public class StructuralGuaranteedPropertyTargetPatternFilter extends
	...
	public StructuralGuaranteedPropertyTargetPatternFilter() {
		this.filterName = "StructuralGuaranteedPropertyTargetPatternFilter"; //$NON-NLS-1$
	}
	...
	protected void initData() {
		...
		for (TypeGraphCondition g : theGTS.getTypes().get(0).getConditions()) {
			if (g.eClass() == SamgraphconditionPackage.eINSTANCE.getNegatedCondition()
					&& isAssumedProperty((NegatedCondition) g)) {
				...
				if (nc.getOperand().eClass() == SamgraphconditionPackage.eINSTANCE.getQuantification()) {
					...
					if (q.getContext().eClass() != SamrulesPackage.eINSTANCE.getRuleGraph()
							|| ((RuleGraph) q.getContext()).getCondition() == null) {
	...
	public void produce() {
		...
		// System.out.println(this.currentInput.second.second.getNodes().size()
		// + "::" + this.currentInput.second.second.getEdges().size());
		if (this.internalHelper.findProperties(this.currentInput.second.second)) {
			discarded++;
			return;
		} else {
			try {
