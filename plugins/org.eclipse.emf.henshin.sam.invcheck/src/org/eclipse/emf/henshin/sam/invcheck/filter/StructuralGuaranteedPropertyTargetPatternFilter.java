...
public class StructuralGuaranteedPropertyTargetPatternFilter extends
	...
	protected void initData() {
		...
		for (TypeGraphCondition g : theGTS.getTypes().get(0).getConditions()) {
			if (g.eClass() == SamgraphconditionPackage.eINSTANCE.getNegatedCondition() && isAssumedProperty((NegatedCondition) g)) {
				...
				for (GraphCondition gc : lgc.getOperands()) {
					if (gc.eClass() == SamgraphconditionPackage.eINSTANCE.getNegatedCondition() && isAssumedProperty(gc)) {
