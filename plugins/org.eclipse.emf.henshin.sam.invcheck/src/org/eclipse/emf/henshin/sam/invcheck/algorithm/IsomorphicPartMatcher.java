...
public class IsomorphicPartMatcher implements AlgorithmComponent {
	 ...
	 * {@link IsomorphicPartMatcher#currentSubGraph}.
	 * 
	 * @author bb
	 *
	 */
	private static class StartConfiguration {
		...
		/**
		 * the {@link IsomorphicPartMatcher#matchingHistory matchingHistories}
		 * index at the time this <code>StartConfiguration</code> has been
		 * created.
		 */
		...
		if (nacms.backtrack == BackTrackReason.FAILURE) {
			...
			if (nacms.mappedNACIter.hasNext()) {
				...
				if (nacSizePredicate(nacms.currentNAC, mappedToNAC)
					 ...
					 * currentMatching.getNodeMatching()) { Node
					 * nodeInTranslatedHost =
					 * ms.translation.getNodeMatching().get(currentMatching.
					 * getNodeMatching().get(entry.getKey())); Node
					 * nodeInPatternNac =
					 * newPatternNacMatch.getNodeMatching().get(currentMatching.
					 * getNodeMatching().get(entry.getKey())); Node
					 * nodeInHostNac = newHostNacMatch.getNodeMatching().get(
					 * nodeInTranslatedHost); if (nodeInPatternNac != null &&
					 * nodeInHostNac != null) {
					 * interfaceMatching.getNodeMatching().put(nodeInPatternNac,
					 * nodeInHostNac); } else if (nodeInPatternNac != null &&
					 * nodeInHostNac == null) { // interfaces do not comply with
					 * positive matching, try another nac return false; } else
	 ...
	 * {@link IsomorphicPartMatcher#setCurrentSubGraph(Set) subgraph} and the
	 * set property attribute of this object. Note, that you have to invoke this
	 * method for each subgraph of a rule Graph to compute all partial matchings
	 * between rule and property.
	 *
