 ...
 * </dl>
 * @author    $Author: jfd $
 * @version   $Revision: 1180 $ $Date: 2012-08-08 15:18:13 +0200 (Mi, 08 Aug 2012) $
 */
...
public class IsomorphicPartPartialMatcher implements AlgorithmComponent
{
	...
	/*
	/**
	 * This is an experimental field to save matchings where a nac in the pattern (or a part of its translation
	 * to the host, to be more exact) does not have an equivalent matching NAC in the host. Consequently, the implication
	 * host => pattern cannot be assured as the absence of the nac in the pattern cannot be assured in the host.
	 * We then save the positive match with the translated NACs for such cases and try to analyse the case
	 * with context generation in another filter. This means, that we treat the translated unmatched NACs as positive
	 * elements (including the original positive elements) and try to find forbidden patterns (without NACs)
	 * in the graph.
	 * 
	 * (Actually, this is not really necessary. We could also check the translated elements directly for the
	 * forbidden patterns. However, this would then be part of the IPM which is probably not really desirable.
	 ...
	 * to start the matching of a connected component of the {@link IsomorphicPartPartialMatcher#currentSubGraph}.
	 *  
	 * @author bb
	 *
	 */
	private static class StartConfiguration {
		...
		/**
		 * the {@link IsomorphicPartPartialMatcher#matchingHistory matchingHistories} index at the time
	...
	private void checkNAC() {
		...
		if (ms.backtrack == BackTrackReason.FAILURE) {
			...
			// check whether bound items in NACs are part of the positive matching so a translation is not necessary
			...
			for (NegativeApplicationCondition nac : SamGraphInvCheckGraphAdapter.getInstance(hostGraph).getNacs()) {				
				...
				for (Annotation an : nac.getAnnotations()) {
					if (!currentMatching.getNodeMatching().containsValue(an.getTarget()) && !currentMatching.getEdgeMatching().containsValue(an.getTarget())) {
						allIn = false;
					}
				}
				if (allIn) {
			...
			if (translate) {
				// if a translation is necessary, it should be done to the union of the positive part of the pattern and the NAC interface in the 
				// host graph.
				...
				// check whether host graph has necessary NACs to imply pattern NACs, by type comparison
				/*
				Map<NodeType, Integer> nodeTypes = new HashMap<NodeType, Integer>();
				for (NegativeApplicationCondition nac : SamGraphInvCheckGraphAdapter.getInstance(hostGraph).getNacs()) {				
					for (Node n : nac.getNodes()) {
						if (nodeTypes.containsKey(n.getInstanceOf())) {
							nodeTypes.put(n.getInstanceOf(), nodeTypes.get(n.getInstanceOf()) + 1);
						} else {
							nodeTypes.put(n.getInstanceOf(), 1);
						}
					}
					for (Annotation an : nac.getAnnotations()) {
					...
					// comparing one NAC should be enough
					break;
				}
				for (Map.Entry<NodeType, Integer> entry : nodeTypes.entrySet()) {
					boolean found = false;
					for (Node n : ms.currentNAC.getNodes()) {
						if (n.getInstanceOf() == entry.getKey()) {
							found = true;
							break;
						}
					}
					if (!found) {
						for (Node n : pattern.getNodes()) {
							if (!this.currentMatching.getNodeMatching().contains(n) && translation.getNodeMatching().get(n) != null && n.getInstanceOf() == entry.getKey()) {
								found = true;
								break;
							}
						}
			...
			while(ms.nacMatching.getNacMatching().size() < tmp.getNacs().size()) {
				if (ms.stack.size() > 0) {
					...
					if (triedAll) {
						// tried all nacs from the host graph to fit in the nac translated from
						// pattern to host.
						// now try to find forbidden graphs (restrictingConstraints) in the nac (nac + pos. elements)
						
						//boolean found = false;
						/*if (this.restrictingConstraints != null) {
							IsomorphicPartPartialMatcher subIpm = new IsomorphicPartPartialMatcher();
							Match m = SubgraphIterator.fullNacToGraph(ms.stack.peek().currentNAC, true);
							Graph newHost = null;
						...
						// this happens when there is no equivalent NAC in the host graph for one (specific) NAC
						// in the translation of the current NAC to the host graph
						ms.backtrack = BackTrackReason.FAILURE;
		...
		if (nacms.backtrack == BackTrackReason.FAILURE) {
			...
			if (nacms.mappedNACIter.hasNext()) {
				...
				if (nacSizePredicate(nacms.currentNAC, mappedToNAC) && nacInterfacePredicate(nacms.currentNAC, mappedToNAC)) {
					...
					// actually, the current NAC representation might be the same if we just check another mappedNAC for the same currentNAC.
					// so we could save the representation instead of calculating it again to save some time.
					Graph newHost = null;
					...
					for (Node n : hostGraph.getNodes()) {
						...
						if (ms.translation != null) {
							//Node nodeInTranslatedHost = ms.translation.getNodeMatching().get(currentMatching.getNodeMatching().get(entry.getKey())); // for translation
							Node nodeInTranslatedHost = ms.translation.getNodeMatching().get(n); // for translation
							//nodeInPatternNac = newPatternNacMatch.getNodeMatching().get(currentMatching.getNodeMatching().get(entry.getKey())); // for translation
							nodeInPatternNac = newPatternNacMatch.getNodeMatching().get(n); // for translation
							nodeInHostNac = newHostNacMatch.getNodeMatching().get(nodeInTranslatedHost); // for translation
						} else {							
							for (Map.Entry<Node, Node> entry : currentMatching.getNodeMatching()) {
								if (entry.getValue() == n) {
									nodeInHostNac = newHostNacMatch.getNodeMatching().get(entry.getKey()); // only for non-translation
									break;
								}
							}
							//nodeInHostNac = newHostNacMatch.getNodeMatching().get(entry.getKey()); // only for non-translation
							//nodeInHostNac = newHostNacMatch.getNodeMatching().get(n); // only for non-translation
							//nodeInPatternNac = newPatternNacMatch.getNodeMatching().get(currentMatching.getNodeMatching().get(entry.getKey())); // for non-translation
							nodeInPatternNac = newPatternNacMatch.getNodeMatching().get(n); // for non-translation
						}
						//Node nodeInHostNac = newHostNacMatch.getNodeMatching().get(entry.getKey()); // only for non-translation
						//Node nodeInPatternNac = newPatternNacMatch.getNodeMatching().get(currentMatching.getNodeMatching().get(entry.getKey())); // for non-translation
						...
						if (nodeInPatternNac != null && nodeInHostNac != null) {
							...
							// this is the case where we can map a negative node from a partial NAC to a positive node
							// from a total NAC. Consequently, we have an empty blacklist.
					...
					if (nacMatching != null) {
						...
						// not sure this works for partial NACs. Probably does not matter. 
	 ...
	 * currently set {@link IsomorphicPartPartialMatcher#setCurrentSubGraph(Set) subgraph} and the
