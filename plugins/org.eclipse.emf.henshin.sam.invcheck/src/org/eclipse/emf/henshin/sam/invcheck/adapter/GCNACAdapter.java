...
public class GCNACAdapter extends EObjectImpl/*NegativeApplicationConditionImpl*/ implements NegativeApplicationCondition {
	
	private NegatedCondition gc;
	private org.eclipse.emf.henshin.sam.model.samgraph.Graph nac;
	
	private static Map<GraphCondition, GCNACAdapter> instances = Collections.synchronizedMap(new WeakHashMap<GraphCondition, GCNACAdapter>());
//	private static Map<GraphCondition, GCNACAdapter> instances = new ConcurrentHashMap<GraphCondition, GCNACAdapter>();
	...
	public static GCNACAdapter getInstance(GraphCondition gc) {
		if (instances.get(gc) == null) {			
			if (gc.eClass() == SamgraphconditionPackage.eINSTANCE.getNegatedCondition()) {
				if (((NegatedCondition)gc).getOperand().eClass() == SamgraphconditionPackage.eINSTANCE.getQuantification()) {
