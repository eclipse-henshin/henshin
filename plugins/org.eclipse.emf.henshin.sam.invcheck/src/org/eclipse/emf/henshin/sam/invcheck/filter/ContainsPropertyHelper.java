 ...
 * The class' visibility is set to <code>package</code> because this class is
...
public class ContainsPropertyHelper {
	...
	public ContainsPropertyHelper(Graph[] props) {
		...
		if (props == null) {
			throw new NullPointerException("null as constructor argument is not permitted");
		}
		this.properties = props;
	}

	/**
	 * An array of <code>Graphs</code> this <code>ContainsPropertyHelper</code>
	 * knows.
	 */
	...
	public Collection<Pair<Graph, Collection<Match>>> allContainedProperties(Graph sourceGraph) {
		this.initIPM(sourceGraph);
		Collection<Pair<Graph, Collection<Match>>> result = new HashSet<Pair<Graph, Collection<Match>>>();
		for (int i = 0; i < this.properties.length; i++) {
			Collection<Match> col = this.findAllOccurences(this.properties[i]);
			if (col != null && !col.isEmpty()) {
				Pair<Graph, Collection<Match>> pair = new Pair<Graph, Collection<Match>>(this.properties[i], col);
				result.add(pair);
			}
		}
		return result;
	...
	public boolean findProperties(Graph sourceGraph) {
		...
		for (Graph prop : this.properties) {
			// System.out.println("trying: " + ((NegatedCondition)
			// prop.eContainer().eContainer()).getName());
			...
			if (this.ipmInstance.getNextMatching() != null) {
				if (SamrulesPackage.eINSTANCE.getRuleGraph().isSuperTypeOf(prop.eClass())
						&& ((RuleGraph) prop).getCondition() != null) {
					// StructuralPropertyFilter.incrementNacDiscard();
					// System.out.println("discarded by: " + ((NegatedCondition)
					// prop.eContainer().eContainer()).getName());
				}
				return true;
			}
	...
	public boolean findPreservedProperties(Graph sourceGraph) {
		...
		for (Node n : sourceGraph.getNodes()) {
			if (((PatternNode) n).getSameInRule() == null
					|| ((PatternNode) n).getSameInRule().eClass() == SamrulesPackage.eINSTANCE.getPreservedNode()) {
				...
				for (Edge out : n.getOutgoing()) {
					...
					if (e.getSameInRule() == null
							|| e.getSameInRule().eClass() != SamrulesPackage.eINSTANCE.getCreatedEdge()) {
						return true;
					}
				}
		...
		for (Edge e : sourceGraph.getEdges()) {
			if (((PatternEdge) e).getSameInRule() == null
					|| ((PatternEdge) e).getSameInRule().eClass() == SamrulesPackage.eINSTANCE.getPreservedEdge()) {
