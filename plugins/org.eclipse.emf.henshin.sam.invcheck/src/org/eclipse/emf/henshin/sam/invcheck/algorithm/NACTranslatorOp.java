...
/**
 * This class translates NACs from a forbidden subgraph to a merged graph which may already contain parts of the NACs.
 * It adds the new NACs to the <a href="../../../../../doc/glossary.html">merged graph</a> ensuring the absence of the NACs from the
 * <a href="../../../../../doc/glossary.html">forbidden subgraph</a> in the result
 * (which is typically a <a href="../../../../../doc/glossary.html">target graph pattern</a>).
 * Thus, any NAC from the forbidden subgraph will not occur as a positive part of the target graph pattern,
 * which would invalidate the counterexample. <br> 
 * <br>
 * To translate NACs from a forbidden subgraph to a merged graph, the following steps have to be done: <br> 
 * <ol>
 * <li>Create a new instance of the class NACTranslator or call {@link #reset()} if an existing instance will be used.</li>
 * <li>Set the forbidden subgraph using the {@link #setPattern(Graph)} method.</li>
 * <li>Set the merged graph using the {@link #setMergedGraph(RuleGraph)} method.</li>
 * <li>Set the initial matching using the {@link #setInitialMatching(Match)} method. It is a matching between the positive items of the
 * forbidden subgraph and the corresponding graph items in the merged graph. This relationship between graph items is first set by the
 * {@link GraphMerger} using the sameInProp field of items in graph patterns.</li>
 * <li>Call the {@link #translate()} method.</li>
 * </ol>
 * 
 * The result is a RuleGraph, usually representing a target graph pattern. This pattern now contains a number of NACs
 * to ensure the absence of nay NACs from the forbidden subgraph so that the target graph pattern is a valid counterexample. <br>
 * <br> 
 * 
 * The following diagram shows the behaviour of the translate method. For further explanation, see {@link #translate()}.
 * 
 * <p><img src="../../../../../doc/diagrams/nacTranslator.png" alt="NAC translation"></p>
 * 
 * @author jfd
 */
public class NACTranslatorOp implements AlgorithmComponent {
	...
	/**
	 * A map mapping a list of partial matchings (from NAC to merged graph) to the start node of the matching process.
	 * This map will be reset each time a new NAC is analyzed.
	 */
	private Map<Node, List<Match>> matchings = new HashMap<Node, List<Match>>();
	
	/**
	 * A list of matching lists built during the translation process. Each matching list contains one matching per
	 * start node. Start nodes include all nodes of the <a href="../../../../../doc/glossary.html">positive NAC interface</a>
	 * and all negative nodes within the NAC. This list will be reset each time a new NAC is analyzed.
	 */
	//private List<List<Match>> listOfLists = new LinkedList<List<Match>>();
	
	
	/**
	 * A matching between the positive items of a forbidden subgraph and their equivalent items in the merged graph.
	 * Needs to be set by calling {@link #setInitialMatching(Match)} before initiating the translation process.
	 * This relationship between the graph items is first set by the {@link GraphMerger} and can be retrieved by checking
	 * the sameInProp field of the graph items.
	 */
	private Match initialMatching;
	
	/**
	 * A matching between the forbidden subgraph and the merged graph. Starting as the initial matching, it is updated
	 * during the translation process. It is used to complete the partial matchings found in the merged graph and thus build
	 * the required NACs.
	 */
	...
	/**
	 * The merged graph the translated NACs will be added to, generating a target graph pattern.
	 */
	private RuleGraph mergedGraph;
	
	/**
	 * This field saves the value of the previous pattern (probably a forbidden pattern) after having executed a partial translate.
	 * We do this in order to shorten further translations, if the possible matches of the NAC elements to be translated do not change
	 * from one merged graph to the next.
	 * 
	 * It might be possible to also integrate this optimization in the execution of a full translate (to be checked). 
	 */
	...
	/**
	 * Sets the initial matching between the positive items of the forbidden subgraph and the merged graph. 
	 * 
	 * @param initialMatching the initial matching
	 */
	 ...
	 * 
	 * @param pattern the forbidden subgraph
	 */
	...
	/**
	 * Resets the current instance of the NAC translator by resetting its data structures.
	 */
	public void reset() {
	...
	public RuleGraph translate(NegativeApplicationCondition singleNac, boolean generateAll) {
		...
		// this determines whether we translate nacs from a rule side or a property
		...
		// this is checked below. not clear, how the final solution will look like, e.g. what is best performance-wise
		/*if (checkFullNacExistence()) {
			return null;
		}*/
		
		// execute for each NAC
		for (NegativeApplicationCondition nac : ((GraphWithNacs) (SamGraphInvCheckGraphAdapter.getInstance(pattern))).getNacs()) {
			...
			// optimization
			// probably do not need that since Iterator automatically optimizes it
			...
			MatchIterator iter = new MatchIterator(g, mergedGraph, interfaceMatch, initialMatching.getNodeMatching().values());
			...
			while(first || iter.hasNext()) {				
				...
				for (Edge e : initialMatching.getEdgeMatching().keySet()) {
					//ignore positive edges from the initial matching because they already have matching edges
				...
				*/
				...
				if (singleNac != null) {
					//System.out.println("sgnumber::match:" + number + "::" + no2);
				}
				...
				/*if (currentMatch == null) {
					emptyMatch = SamtraceFactory.eINSTANCE.createMatch();
				...
				for (Edge e : g.getEdges()) {
					...
					if (gm.getNodeMatching().containsKey(e.getSource())) {
						...
						// do not think we need this as all nodes should be in gm
					...
					for (Edge matchingEdge : this.mergedGraph.getEdges()) {
						if (!initialMatching.getEdgeMatching().containsValue(matchingEdge) && !gm.getEdgeMatching().containsValue(matchingEdge)) {
							if (matchingEdge.getTarget() == tar && matchingEdge.getSource() == src) {
								if (matchingEdge.getInstanceOf() == e.getInstanceOf()) {
									...
									break;
									// note that there may be more than one matching edge which is not considered here
									// however, all matching edges share the same type and source/target,
									// so they are equivalent.
								}	
							} else if (matchingEdge.getTarget() == src && matchingEdge.getSource() == tar) {
								if (matchingEdge.getInstanceOf() == e.getInstanceOf() && e.getInstanceOf().getDirection() == EdgeDirection.UNDIRECTED) {
									// undirected edge - src and tar may be reversed for the matching edge
									resultEdge = matchingEdge;
									break;
									// note that there may be more than one matching edge which is not considered here
									// however, all matching edges share the same type and source/target,
				...
				if (g.getNodes().size() - interfaceMatch.size() <= gm.getNodeMatching().size() && g.getEdges().size() == gm.getEdgeMatching().size()) {						
					return null;
				}
				...
				for (Node n : nac.getNodes()) {
					...
					if (copyInMergedGraph != null && currentMatching.getNodeMatching().containsValue(copyInMergedGraph)) { // new								
						inconsistent = true;			
						break;
					}
					...
					if (copyInMergedGraph != null) {
						// node matches another node in the merged Graph, so add the match						
				...
				if (inconsistent) {					
					// there were inconsistent matchings in this element of the cartesian product, so skip the list
					//cnt++;
					newNac.setGraph(null);						
					continue;
				}
				...
				for (Edge e : nac.getEdges()) {
					...
					if (copyInMergedGraph != null && currentMatching.getEdgeMatching().containsValue(copyInMergedGraph)) {
						inconsistent = true;
					...
					if (copyInMergedGraph != null) {
						...
						// This is an attempt to keep track of the new items created in a NAC
						// so that redundant NACs can be skipped. Since it did not work out as planned,
				...
				for (Map<AnnotatedElem, AnnotatedElem> curr : srcItemsm.values()) {
					...
					if (curr.size() >= currentNewm.size()) {
						// existing NAC could be stronger (smaller negative part) than currentNac
						...
						// Now we check whether there exists an item matched as positive in the existing NAC
						// that is matched to a different positive element or not at all in the current NAC.
						// If the existing NAC is smaller, meaning that the positive matching is larger,
						// this will obviously be the case. If they are of equal size and share the same elements, they 
						// can only be (I think) from the same subgraph and the same match, so this will not happen.
						 
						// before changing the subgraph iterator to disregard permutations of the NAC interface in subgraphs,
						// it was possible to gain equivalent matches from those different subgraphs. After changing that, it
				...
				//System.out.println("remove");
					...
					// iterate over all positive matches for existing NACs to be added
					for (NegativeApplicationCondition nacToReplace : srcItemsm.keySet()) {
						...
						// the current NAC is smaller than the existing NAC -> check if existing NAC can be replaced
						if (currentNewm.size() >= currItems.size()) {
							remove = true;
							for (Map.Entry<AnnotatedElem, AnnotatedElem> entry : currentNewm.entrySet()) {
								// check whether there is an item matched to a positive item in the current NAC
								// that is not matched in the existing NAC. If there is such a node, we must keep
								// the existing NAC. If it is an edge, the NAC might be removed.
								if (!currItems.containsKey(entry.getKey())) {
									if (!SamgraphPackage.eINSTANCE.getEdge().isSuperTypeOf(entry.getKey().eClass())) {
										remove = false;
										break;
									}
									// if there is an item in the current positive match that is matched differently in the existing NAC,
					...
					//GraphCondition nc = InvariantCheckingUtil.createNegatedCondition(newNac);
					//reduceNacs(nc, mergedGraph);
					/*
					if (mergedGraph.getCondition() == null) {
						mergedGraph.setCondition(nc);
					} else if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getNegatedCondition()) {
						LogicalGCCoupling co = SamgraphconditionFactory.eINSTANCE.createLogicalGCCoupling();
						co.getOperands().add(mergedGraph.getCondition());
						co.getOperands().add(nc);
						co.setOperator(LogicalOperator.CONJUNCTION);
						mergedGraph.setCondition(co);
					} else if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getLogicalGCCoupling()) {
						((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().add(nc);
					}*/
					//((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().add(nc);
					//mergedGraph.setCondition(InvariantCheckingUtil.addNegatedCondition(mergedGraph.getCondition(), newNac));
	...
	/*if (mergedGraph.getCondition() != null) {
		if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getLogicalGCCoupling()) {
			System.out.println("size of nacs: " + ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size());
		} else {
	 ...
	 * 
	 * This method checks whether we do not need to find matches for a specific subgraph of a nac
	 * because all possible translated NACs would result in a forbidden pattern.
	 * 
	 * Later, it may be possible to do this for subgraphs with all possible edges so that all
	 * subgraphs of that subgraph can also be discarded without having to be checked individually. 
	 * 
	 * @param subgraph
	 * @param nac
	 * @return
	...
	private boolean checkSubgraphValidity(Set<AnnotatedElem> subgraph, NegativeApplicationCondition nac, Match nacConversion) {
		...
		for (Edge e : nac.getEdges()) {
			if (!subgraph.contains(nacConversion.getEdgeMatching().get(e)) && !subgraph.contains(nacConversion.getNodeMatching().get(e.getSource())) && !subgraph.contains(nacConversion.getNodeMatching().get(e.getTarget()))) {
				Edge newEdge = e.copy();
				...
				if (nac.getNodes().contains(e.getSource())) {
					...
					newEdge.setSource(tmpMatch.getNodeMatching().get(initialMatching.getNodeMatching().get(e.getSource())));
				}
				if (nac.getNodes().contains(e.getTarget())) {
					newEdge.setTarget(nacToHost.getNodeMatching().get(e.getTarget()));
				} else {
					newEdge.setTarget(tmpMatch.getNodeMatching().get(initialMatching.getNodeMatching().get(e.getTarget())));
				}				
			}
		}
	...
	public RuleGraph translateContext() {
		...
		for (NegativeApplicationCondition nac : ((GraphWithNacs) (SamGraphInvCheckGraphAdapter.getInstance(pattern))).getNacs()) {
			...
			for (Map.Entry<Node, Node> entry : initialMatching.getNodeMatching()) {
				...
				if (!isInterfaceNode) {
					if (nodeTypeMap.containsKey(nodeInPattern.getInstanceOf())) {
						nodeTypeMap.put(nodeInPattern.getInstanceOf(), nodeTypeMap.get(nodeInPattern.getInstanceOf()) + 1);
					} else {
			...
			OptimizedSubgraphIterator sgIter = new OptimizedSubgraphIterator(g, mergedGraph, nodeTypeMap, edgeTypeMap, interfaceNodes);
			ipm.setPattern(g);
			...
			while(sgIter.hasNext()) {				
				...
				if (!checkSubgraphValidity(subgraph, nac, nacConversion)) {
					...
					sgIter.skip();
						// if we skip at this point, we consider only subgraphs consisting of nodes.
						// edges are not considered. This might be a possible optimization, though I
						// still need to find out in what way.
						
						//sgIter.skip();
						
						// this is more general, probably also applicable to normal NAC translation:
						// generate only subgraphs consisting solely of nodes, create all NACs.
						// Afterwards, iterate over all edges and try to map as many of them as possible.
						// Their position is unambiguously defined by their source and target nodes.						
						// Thus, we avoid generating all combinations of edgse absent or present for 
						// a given subgraph consisting of nodes.
						
						// This approach should probably be formally justified. We would need to prove
						// that a nac without a specific edge is more powerful than the same NAC
						// containing the edge.
						
						// Using only nodes might lead to more time being spent in the IPM algorithm
						// since nodes are not connected and thus need to be matched in separate
						// connected components. Consequently, the algorithm to find start nodes in
						// connected components should be improved, e.g. by consideration of node types.
		...
		/*if (singleNac != null) {
			...
			System.out.println("incon: " + incon);
			System.out.println("matches: " + matches);
		}*/
		
		if (mergedGraph.getCondition() != null) {
			if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getLogicalGCCoupling()) {
				//System.out.println("nonpartial, size of nacs after reduce: " + ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size() + ", size: " + mergedGraph.getNodes().size());
				size = ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size();
			} else {
				//System.out.println("nonpartial, size of nacs after reduce: " + 1);
			}
		}
		return mergedGraph;
	}
	...
	public boolean checkFullNacExistence() {
		for (NegativeApplicationCondition nac : ((GraphWithNacs) (SamGraphInvCheckGraphAdapter.getInstance(pattern))).getNacs()) {
			...
			// search for full nac to prevent target pattern that will trivially evaluate to false
	...
	/**
	 * In contrast to the nonpartial translate, partialTranslate does not use fixed nodes in the SubgraphIterator.
	 * This is mainly due to the fact that the NACTranslationFilter does not take that much time when using
	 * partial translates.
	 * 
	...
	public Graph partialTranslate(GraphRule rule) {
		...
		/*
		 * new approach for NAC translation:
		 * - during reverse rule application, add deleted items to (positive) items in the partial NAC
		 * - when partially translating, build the subgraph from the partial NAC with:
		 * 		- edges with a created type
		 * 		- nodes with a created type
		 * 		- nodes adjacent to edges with a created type  
		 * - build the target graph to match to (usually the merged graph) with:
		 * - created nodes
		 * - created edges
		 * - nodes adjacent to created edges
		 * 
		 * It is probably necessary to extract the respective subgraph from the merged graph before executing the translation. As long as the forbidden 
		 * pattern and the subgraph as constructed above are disjoint, the NACs should be the same.
		 * 
		 * When translating, the items marked as bound items are not necessarily only targets of node or edge mappings but also those
		 * that were included as potential target before starting the mapping/translation process. This is similar to the way the complete NAC
		 * is copied to the merged graph in the straight-forward full translation without matching a single element while still being represented by
		 * a total, not a partial morphism.
		 * 
		 * This means that the annotations regarding bound items are the same for each partially translated NAC, assuming that they were created 
		 * as described above.
		 * 
		 */
		...
		for (NegativeApplicationCondition nac : ((GraphWithNacs) (SamGraphInvCheckGraphAdapter.getInstance(pattern))).getNacs()) {
			...
			// save the graph containing the negative elements. This is for caching purposes only.
			Graph nacGraph = null;
			for (Node n : nac.getNodes()) {
			...
			// before setting the hostGraph attribute of the IPM, we need to calculate the graph to translate the NAC to.
			// Since this is a partial translation, we translate only to nodes/edges that are created, nodes adjacent to created edges and preserved nodes adjacent to deleted edges.
			// For caching purposes we also save the equivalent nodes/edges in the rule.
			// actually, we could probably restrict the subgraphMergedGraph even further since only items with types from the NAC will be relevant
			...
			for (Node n : mergedGraph.getNodes()) {
				if(((PatternNode) n).getSameInRule() != null && ((PatternNode) n).getSameInRule().eClass() == SamrulesPackage.eINSTANCE.getCreatedNode()) {
					extractedNodes.add(n);
					createdNodeTypes.add(n.getInstanceOf());
					extractedNodesInRule.add(((PatternNode) n).getSameInRule());
				}
				
				// need to add nodes adjacent to edges that are deleted by the rule, i.e. created by the reverse rule application
			...
			for (Edge e : mergedGraph.getEdges()) {
				if(((PatternEdge) e).getSameInRule() != null && ((PatternEdge) e).getSameInRule().eClass() == SamrulesPackage.eINSTANCE.getCreatedEdge()) {
			...
			// problem here: nac interface not included in subgraphMErgedGraph (extracted), therefore illegal matches on
			// interface elements are found
			// solution: first, choose elements of NAC that will be given to the subgraph iterator.
			// Then, for any NAC interface elements contained, add the respective elements in the subgraphMErgedGraph.
			...
			//OptimizedSubgraphIterator sgIter = new OptimizedSubgraphIterator(g);
			...
			OptimizedSubgraphIterator sgIter = new OptimizedSubgraphIterator(g, false, subgraphMergedGraph, null, null, convertedInterfaceNodes);
			
			ipm.setPattern(g);
			//ipm.setHostGraph(mergedGraph); // this actually has to be the modified graph
			...
			for (Map.Entry<Node, Node> entry : initialMatching.getNodeMatching()) {
				...
				if (nodeInNac != null && extractedSGMatch.getNodeMatching().containsKey(entry.getValue())) {
					nacInterfaceMatch.getNodeMatching().put(nodeInNac, extractedSGMatch.getNodeMatching().get(entry.getValue()));
				}
			}
			...
			// search for full nac to prevent target pattern that will trivially evaluate to false
			// will skip this for the moment. Need to evaluate whether it is better to check it afterwards in case of a
			...
			if (currentMatch1 != null) {
				...
				//System.out.println("full nac found: " + ((NegatedCondition) pattern.eContainer().eContainer()).getName());
				return null;				
			}
			ipm.reset();
			ipm.setPattern(g);
			...
			// ... and also all extracted elements from the merged graph the NAC was translated to
			...
			if (rule == this.previousRule && this.previousNacGraph == nacGraph) {
				repeat = true;
				if (this.previousRuleEdges != null && this.previousRuleEdges.size() == extractedEdgesInRule.size() && this.previousRuleNodes != null && this.previousRuleNodes.size() == extractedNodesInRule.size()) {
			...
			if (!repeat) {
				...
				for (Match m : this.previousMatches) {
					...
					for (Map.Entry<Node, Node> entry : m.getNodeMatching()) {
						...
						for (Map.Entry<Node, Node> nCEntry : this.previousNacConversion.getNodeMatching()) {
							if (nCEntry.getValue() == nodeInOldSubgraph) {
								...
								// we cannot use mappings involving the merged graph because merged graphs change between translations
								// so we have to look at the sameInRule item
								// in theory, every mapped item should have a sameInRule equivalent because we cannot map nac items on items from the property itself
								
								// this can probably be done easier if we save our matches in a different form (e.g., direct mappings to mergedGraph/rule instead of mappings to intermediate graphs)
								Node nodeInOldMerged = this.previousMirroredExtractedMatch.getNodeMatching().get(entry.getValue());
								...
								//gm.getNodeMatching().put(nodeInCurrent, extractedSGMatch.getNodeMatching().get(this.previousMirroredExtractedMatch.getNodeMatching().get(entry.getValue())));
								break;
							}
						}
					}
					...
					/*
					boolean stop = false;
					if (gm.getNodeMatching().size() == 7 && mergedGraph.getNodes().size() == 16 && mergedGraph.getEdges().size() == 27) {
						stop = true;
						for (Map.Entry<Node, Node> entry : gm.getNodeMatching()) {
							if (!entry.getKey().getName().substring(1).equals(entry.getValue().getName())) {
								System.out.println(entry.getKey().getName() + " :: " + entry.getValue().getName());
								stop = false;
								break;
							}
					...
					for (Edge e : g.getEdges()) {
						...
						for (Edge matchingEdge : subgraphMergedGraph.getEdges()) {
							if (!initialMatching.getEdgeMatching().containsValue(mirroredExtractedSGMatch.getEdgeMatching().get(matchingEdge)) && !gm.getEdgeMatching().containsValue(matchingEdge)) {
								if (matchingEdge.getTarget() == tar && matchingEdge.getSource() == src) {
									if (matchingEdge.getInstanceOf() == e.getInstanceOf()) {
										resultEdge = matchingEdge;
										break;
					...
					if (gm.getNodeMatching().size() >= 7) {
						//System.out.println("nodes: " + gm.getNodeMatching().size() + ", edge: " + gm.getEdgeMatching().size());
					}
					...
					for (Node n : nac.getNodes()) {
						Node copyInNewGraph = nacConversion.getNodeMatching().get(n);
						Node copyInMergedGraph = null;
						copyInMergedGraph = mirroredExtractedSGMatch.getNodeMatching().get(gm.getNodeMatching().get(copyInNewGraph));
						if (copyInMergedGraph != null && currentMatching.getNodeMatching().containsValue(copyInMergedGraph)) { // new								
							inconsistent = true;
							break;
						}
						...
						if (copyInMergedGraph != null) {
							...
							// create annotation that will indicate whether node is "repeated" in the corresponding NAC
							// the annotations will be saved in the NAC itself, not in the items. This is not an ideal solution, but
							// will hopefully suffice until I come up with a better idea
							/*Annotation an = SamannotationFactory.eINSTANCE.createAnnotation();
							an.setSource(InvariantCheckingCore.NAC_BOUND_ITEM);
					...
					for (Edge e : nac.getEdges()) {
						Edge copyInNewGraph = nacConversion.getEdgeMatching().get(e);
						Edge copyInMergedGraph = null;
						copyInMergedGraph = mirroredExtractedSGMatch.getEdgeMatching().get(gm.getEdgeMatching().get(copyInNewGraph));
						if (copyInMergedGraph != null && currentMatching.getEdgeMatching().containsValue(copyInMergedGraph)) {
							inconsistent = true;
							break;
						}
			...
			while(sgIter.hasNext()) {				
				...
				while (currentMatch != null/* || emptyMatch == null*/) {
					...
					// save rule, pattern and extractetMergedSubgraph and compare
					// note that we should not compare extractedMergedSubgraphs, but the items used in creating them
					...
					// match as many edges as possible
					// note that the partial translate function does not use subgraph optimization,
					// meaning that there will also be subgraphs consisting of interface nodes? probably?
		...
		if (mergedGraph.getCondition() != null) {
			if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getLogicalGCCoupling()) {
				//System.out.println("partial, size of nacs: " + ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size() + ", node size: " + mergedGraph.getNodes().size());
			} else {
		...
		if (mergedGraph.getCondition() != null) {
			if (mergedGraph.getCondition().eClass() == SamgraphconditionPackage.eINSTANCE.getLogicalGCCoupling()) {
				//System.out.println("partial, size of nacs: " + ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size() + ", " + ((NegatedCondition) pattern.eContainer().eContainer()).getName());
				size = ((LogicalGCCoupling) mergedGraph.getCondition()).getOperands().size();
			} else {
				//System.out.println("partial, size of nacs: " + 1);
			}
