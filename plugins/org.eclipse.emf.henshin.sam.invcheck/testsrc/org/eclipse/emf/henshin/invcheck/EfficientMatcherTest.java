...
import org.eclipse.emf.henshin.sam.invcheck.algorithm.EfficientMatcher;
...
public class EfficientMatcherTest extends TestCase {
	...
	private void buildHost1() {
		...
		h1n4 = buildNode("b2", "B", host);
		h1n5 = buildNode("b3", "B", host);
		h1n6 = buildNode("b4", "B", host);
		h1n7 = buildNode("c1", "C", host);
		
		h1e1 = buildEdge("AB", host);
		h1e1.setSource(h1n1);
		h1e1.setTarget(h1n3);
		h1e1 = buildEdge("AB", host);
		h1e1.setSource(h1n1);
	...
	private void buildPattern1() {
		...
		t1n3 = buildNode("b1", "B", target);
		t1n4 = buildNode("b2", "B", target);
		t1n5 = buildNode("b3", "B", target);
		t1n6 = buildNode("b4", "B", target);
		t1n7 = buildNode("c1", "C", target);
		
		t1e1 = buildEdge("AB", target);
		t1e1.setSource(t1n1);
		t1e1.setTarget(t1n3);
		t1e1 = buildEdge("AB", target);
		t1e1.setSource(t1n1);
	...
	public void testTestIterator2() {
		...
		EfficientMatcher matcher = new EfficientMatcher(host, target, null);
		int matches = 0;
		Match current = matcher.getNextMatching();
		while (current != null) {			
			matches++;
			...
			System.out.println();
			current = matcher.getNextMatching();
		}
	...
	public void testCompOldAndNew() {
		...
		ipm.setPattern(host);
		Set<AnnotatedElem> subgraph = OptimizedSubgraphIterator.graphToSubGraph(host);
		ipm.setCurrentSubGraph(subgraph);		
		
		int matches = 0;		
		Match current = ipm.getNextMatching();
		while (current != null) {
			matches++;
			current = ipm.getNextMatching();
		}
	...
	public void testCompOldAndNew2() {
