...
/**
 * Assertions for (static) {@link EGraph}s. For everything related to changes (caused by rules / transformation units)
 * on graphs, see {@link GraphTransformations}.
 * 
...
public class Graphs {
	...
	public static void assertObjectNotInGraph(EObject obj, EGraph graph) throws AssertionError {
		if (graph.contains(obj)) {
			throw new AssertionError("expected: Object " + obj.toString() + " not contained in graph");
		}
	}
	 ...
	 * <ul>
	 * <li>all matched elements have a similarity above the matchSimilarityThreshold</li>
