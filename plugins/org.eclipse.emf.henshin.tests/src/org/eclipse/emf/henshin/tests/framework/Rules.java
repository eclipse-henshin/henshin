...
/**
 * Assertions for Rules. Contains statistical assertions for matches (i.e. hasMatch, hasNoMatch, etc.). For assertions
 * on specific objects matched, see {@link Matches}.
 * 
 * @see Matches
 * @see Units
 * @author Felix Rieger
...
public class Rules {
	 ...
	 */
	public static void assertRuleHasMatch(Rule rule, EGraph graph, Match partialMatch, Engine engine)
			throws AssertionError {
	 ...
	 * @throws AssertionError
	 */
	public static void assertRuleHasNMatches(Rule rule, EGraph graph, Match partialMatch, Engine engine, int n)
			throws AssertionError {
		int found = InterpreterUtil.findAllMatches(engine, rule, graph, partialMatch).size();
		if (found != n) {
			throw new AssertionError("expected: Rule " + rule.getName() + " has " + n + " matches, but has " + found);
		}
	}
	...
	public static void assertRuleCanBeAppliedMultipleTimes(RuleApplication ra) throws AssertionError {
		...
		if (!success) {
			throw new AssertionError("expected: Rule " + ra.getRule().getName() + " can be applied multiple times.");
		}
	}
	...
	public static void assertRuleCanBeAppliedNTimes(RuleApplication ra, int n) throws AssertionError, Exception {
		...
		if (!success) {
			throw new AssertionError("expected: Rule " + ra.getRule().getName() + " can be applied at least " + n
					+ " times");
		}
	}
