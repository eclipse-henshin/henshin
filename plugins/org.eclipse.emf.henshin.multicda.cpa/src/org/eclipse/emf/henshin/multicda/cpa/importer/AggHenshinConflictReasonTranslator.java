...
import org.eclipse.emf.henshin.multicda.cpa.result.ConflictElement;
 ...
 *
 */
public class AggHenshinConflictReasonTranslator {
	...
	private List<ConflictElement> criticalElements;
	 ...
	 */
	public AggHenshinConflictReasonTranslator(List<org.eclipse.emf.henshin.model.Rule> rules) {
	...
	/**
	 * Processes a single conflict reason of AGG results to obtain this result as part of the <code>CPAResult</code>.
	 * 
	 * @param result The finally provided <code>CPAResult</code> of Henshins conflict reason analysis.
	 * @param rule1 The first rule of AGG of the conflict reason.
	 * @param rule2 The second rule of AGG of the conflict reason.
	 * @param cpd The container of AGG containing the conflict reason.
	 */
	private void processAGGresultOfRulePair(CPAResult result, Rule rule1, Rule rule2, CriticalPairData cpd) {

		criticalElements = new LinkedList<ConflictElement>();
		...
		// initialize all the resulting objects of a single conflict reason: -->
		...
		ConflictGraphMapping criticalGraphMapping = new ConflictGraphMapping();
		...
		for (int i = 0; i < morph1SourceObjects1.size(); i++) {
			...
			if (morph1TargetObject.isNode()) {
				...
				if (morph1TargetObject.isCritical()) { // ensures the highlighting of the critical element
					...
					ConflictElement criticalElement = new ConflictElement();
							...
							for (Attribute henshinRhsAttribute : henshinNodeRhs.getAttributes()) {
								...
								if (attributeChanged) {
									if (!anyAttributeProcessed) {
										...
										ConflictElement criticalElementforFurtherChangedAttribute = new ConflictElement();
	 ...
	 * 
	 * @param criticalPairData The result of AGG for this current conflict reason.
	...
	private void processCriticalElementOfSecondRule(CriticalPairData criticalPairData, GraphObject morph2TargetObject,
		...
		for (ConflictElement existingCritElem : criticalElements) {
	 ...
	 * 
	 * @param sequentialRule The information, if it is the first or the second rule of the conflict reason.
	 ...
	 *            in the two Henshin rules forming the conflict reason.
	 */
	private void extractNodeAssignmentsOfNestedConditions(SequentialRule sequentialRule,
			EList<NestedCondition> nestedConditions, ConflictGraphMapping criticalGraphMapping) {
	...
	/**
	 * Processes a single edge of AGGs conflict reason result.
	 * 
	 ...
	 * @param conflictGraphMapping The mapping of the AGG overlap graph, also called minimal model of AGG and the nodes
	 *            in the two Henshin rules forming the conflict reason.
	...
	private void processEdgeOfAGGResult(GraphObject morphismTargetObject, SequentialRule sequentialRule,
			boolean arcIsCritical, ConflictGraphMapping conflictGraphMapping) throws Exception {
		...
		if (arcIsCritical) {
			if (sequentialRule == SequentialRule.FirstRule) {
				ConflictElement confEdgeElem = new ConflictElement();
				criticalElements.add(confEdgeElem);
				Node henshinSourceNode = conflictGraphMapping.getFirstRuleNode(sourceNode);
				Node henshinTargetNode = conflictGraphMapping.getFirstRuleNode(targetNode);
				...
				for (Edge edge : henshinSourceNode.getOutgoing()) {
					if (edge.getTarget() == henshinTargetNode) { // correct edge found
						confEdgeElem.elementInFirstRule = edge;
						confEdgeElem.commonElementOfCriticalGraph = morphismTargetObject;
					}
				}
			...
			if (sequentialRule == SequentialRule.SecondRule) {
				Node henshinSourceNode = conflictGraphMapping.getSecondRuleNode(sourceNode);
				Node henshinTargetNode = conflictGraphMapping.getSecondRuleNode(targetNode);
				// if (henshinSourceNode == null || henshinTargetNode == null)
				// System.err.println("WARNING! - cant process the critical edge '" + morphismTargetObject.toString()
				// + "' since related henshin node cant be resolved.");
				...
				if (correspondingHenshinEdge == null) {
					List<Node> potentialHenshinSourceNodesOfNestedConditions = conflictGraphMapping
							.getSecondRuleNodesOfNestedGraphs(sourceNode);
					List<Node> potentialHenshinTargetNodesOfNestedConditions = conflictGraphMapping
					...
					for (Node potentialSourceNode : potentialHenshinSourceNodesOfNestedConditions) {
						for (Edge edge : potentialSourceNode.getOutgoing()) {
							if (conflictGraphMapping.getSecondRuleNode(targetNode) == edge.getTarget()
				...
				if (correspondingHenshinEdge != null) {
					for (ConflictElement critElem : criticalElements) {
	...
	/**
	 * Extracts the kind of the conflict reason out of AGGs <code>CriticalPairData</code>.
	 * 
	...
	/**
	 * transforms AGG's kind of conflict reason (from <code>cpd</code>) into our own dependency kinds
	 * 
	 * @param cpd The conflict reason data from AGG.
	...
	/**
	 * transforms AGG's kind of conflict reason (from <code>cpd</code>) into our own conflict kinds
	 * 
	 * @param cpd the conflict reason data from AGG
