...
import org.eclipse.emf.henshin.multicda.cpa.importer.AggHenshinConflictReasonTranslator;
...
/**
 * The Implementation of the conflict reason analysis for the <code>ICriticalPairAnalysis</code> interface by using the
 * analysis implemented in AGG.
 * 
 * @author Kristopher Born
 *
 */
public class CpaByAGG implements IConflictReasonAnalysis {
	GraGra gragra;
	Module module;

	AggHenshinConflictReasonTranslator importer;
	...
	/**
	 * A constructor which allows to store the result of the conflict reason analysis by AGG within AGGs format.
	 * 
	 * @param debugFile The File for storing the result of the conflict reason analysis by AGG within AGGs format.
	 */
	...
	/**
	 * Initializes the conflict reason calculation with the <code>rules</code> parameter, used as first and second rules
	 * as well as the <code>options</code>. Even performs a check to ensure the parameters full fill the requirements to
	 * perform the analysis.
	 * 
	 * @param rules The rules for which the conflict reason analysis shall be executed.
	 * @param options The options settings that shall be applied on the calculation of the conflict reasons
	...
	/**
	 * Initializes the conflict reason calculation with <code>r1</code> as first rules and <code>r2</code> as second rules
	 * of the conflict reasons as well as the <code>options</code>. Even performs a check to ensure the parameters full
	 * fill the requirements to perform the analysis.
	 * 
	 * @param r1 the first rules for the conflict reason analysis.
	 * @param r2 the second rules for the conflict reason analysis.
	 * @param options the options settings that shall be applied on the calculation of the conflict reasons.
	 * @throws UnsupportedRuleException in case of invalid rules.
	 */
	@Override
	public void init(List<org.eclipse.emf.henshin.model.Rule> r1, List<org.eclipse.emf.henshin.model.Rule> r2,
		...
		importer = new AggHenshinConflictReasonTranslator(originalRules);
	 ...
	 * @return <code>true</code> if the rules are valid for conflict reason analysis.
	 * @throws UnsupportedRuleException in case of invalid rules.
	 */
	@Override
	 ...
	 * @return a set of conflict reason results which are conflicts.
	 */
	@Override
	 ...
	 * 
	 * @return a set of conflict reason results which are dependencies.
	 */
	@Override
	...
	/**
	 * compute the conflict reason matrix between the two lists. in the case of N rules1 and M rules2, only N*M + M*N
	 ...
	 * 
	 * @param epc The container with the rules, options and many more within AGG for calculating the conflict reasons.
