...
import org.eclipse.emf.henshin.multicda.cpa.persist.ConflictReasonNode;
...
/**
 * Utility for the conflict reason analysis. Up till now mainly used for persisting the results in the file system.
 * 
...
public class CPAUtility {
	...
	/**
	 * Persists the results of a conflict reason analysis in the file system.
	 * 
	 * @param cpaResult A <code>CPAResult</code> of a conflict reason analysis.
	 * @param path The path for saving the full result set.
	 * @return a <code>HashMap</code> of the saved results.
	 */
	public static HashMap<String, Set<ConflictReasonNode>> persistCpaResult(CPAResult cpaResult, String path) {
		...
		HashMap<String, Set<ConflictReasonNode>> persistedCPs = new HashMap<String, Set<ConflictReasonNode>>();

		for (ConflictReason cp : cpaResult) {
			...
			if (persistedCPs.containsKey(folderName)) {
				...
				persistedCPs.put(folderName, new HashSet<ConflictReasonNode>());
			}
			...
			// persist a single conflict reason.
			ConflictReasonNode newCriticalPairNode = persistSingleCriticalPair(cp, numberedNameOfCPKind,
					pathWithDateStamp);
	...
	/**
	 * Persists a single conflict reason (<code>cp</code>) in the file system.
	 * 
	 * @param cp The <code>CriticalPair</code> to be saved.
	 * @param numberedNameOfCriticalPair The numbered name of the conflict reason.
	 * @param path The path for saving the files.
	 * @return a <code>CriticalPairNode</code>.
	 */
	private static ConflictReasonNode persistSingleCriticalPair(ConflictReason cp, String numberedNameOfCriticalPair,
			String path) {
		...
		return new ConflictReasonNode(numberedNameOfCriticalPair, firstRuleURI, secondRuleURI, overlapURI,
				criticalPairURI);
	}
	 ...
	 * 
	 * @param minimalModel The <code>EGraph</code>, which should be a minimal model of a conflict reason.
...
//				HenshinDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
	 ...
	 * 
	 * @param minimalModel The minimal model of the conflict reason.
	...
	private static URI saveCriticalPairInFileSystem(ResourceSet resourceSet,
			org.eclipse.emf.henshin.multicda.cpa.conflictreason.IConflictReason criticalPairOfThisProcess, String fullPath) {
