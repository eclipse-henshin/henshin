...
public class CTMCStateSpaceValidator extends AbstractFileBasedValidator {
	...
	public ValidationResult validate(StateSpace stateSpace, IProgressMonitor progressMonitor) throws Exception {
		...
		SubMonitor monitor = SubMonitor.convert(progressMonitor, "Checking CSL property...", 4);

		// Generate the model file.
		CTMCStateSpaceExporter exporter = new CTMCStateSpaceExporter();
		File modelFile = export(stateSpace, exporter, null, "sm", monitor.split(1));
		...
		Process process = PRISMUtil.invokePRISM(stateSpace, modelFile, cslFile, null, constants, true, monitor.split(1));
		
		// Parse the experiments:
		return PRISMExperiment.parseValidationResult(stateSpace, process, monitor.split(1));
