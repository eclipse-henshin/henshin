...
/**
 * This checker provides a set of checks to ensure that the rules are suitable
 * for the current implementation of the critical pair analysis.
 * 
...
public class InputDataChecker {
	...
	/**
	 * Checks the list of rules in regard of being suitable for applying this
	 * critical pair analysis implementation.
	 * 
	 * @param rules
	 *            The rules to be checked in regard of being suitable for
	 *            applying this critical pair analysis implementation.
	 * @return <code>true</code> if all of the rules are suitable for applying
	 *         this critical pair analysis implementation.
	 * @throws UnsupportedRuleException
	 *             On the case of any unsuitable rules.
	 */
	public boolean check(List<org.eclipse.emf.henshin.model.Rule> rules)
			throws UnsupportedRuleException {
		...
		return sameDomainModel && consistentOptions
				&& allAttributeTypesSupported && noAmalgamation
				&& allApplicationConditionsSupported;
	}

	/**
	 * Checks the rules for the same domain model based on the nsUri. This check
	 * depends on a reliable nsUri, which is being adjusted on editing the
	 * metamodel. A stronger check could use EMFcompare to check the metamodels.
	 * 
	 * @param rules
	 *            The rules which are checked regarding a common meta model.
	 * @return <code>true</code> if all of the rules are based on the same meta
	 *         model.
	 * @throws DifferentDomainException
	 *             On any occurrence of different meta models within the set of
	 *             rules.
	 * @throws MultipleDomainImportException
	 *             On any occurrence of more than one meta model in a rule.
	 */
	private boolean checkDomainModels(
		...
		for (org.eclipse.emf.henshin.model.Rule rule : rules) {
			...
			if (rule.getModule().getImports().size() == 0)
				throw new UnsupportedRuleException(
						UnsupportedRuleException.missingDomainImport);
			if (rule.getModule().getImports().size() > 1)
				throw new UnsupportedRuleException(
						UnsupportedRuleException.multipleDomainImport);
			if (nsUri == null) {
				nsUri = rule.getModule().getImports().get(0).getNsURI(); // TODO:
																			// NPE,
																			// wenn
																			// die
																			// Anzahl
																			...
																			// null
																			// ist.
			} else if (!rule.getModule().getImports().get(0).getNsURI()
					.equals(nsUri)) {
				throw new UnsupportedRuleException(
						UnsupportedRuleException.differentDomain);
			}
		}
		return true;
	}
	 ...
	 * 
	 * @param rules
	 *            The rules which are checked for containing amalgamation.
	 * @return <code>true</code> if all of the rules do not contain any
	 *         amalgamation.
	 * @throws UnsupportedRuleException
	 *             On any found amalgamation within the set of rules.
	 */
	private boolean checkForAmalgamation(
			List<org.eclipse.emf.henshin.model.Rule> rules)
			throws UnsupportedRuleException {
		for (org.eclipse.emf.henshin.model.Rule rule : rules) {
			if (rule.getMultiRules().size() > 0)
				throw new UnsupportedRuleException(
						UnsupportedRuleException.unsupportedAmalgamationRules);
		}
		return true;
	}
	 ...
	 * 
	 * @param rulesToBeChecked
	 *            The rules, of which the contained attributes are checked
	 *            regarding their data types.
	 * @return <code>true</code> if all the contained attributes are supported.
	 * @throws UnsupportedRuleException
	 *             On found data types which are not supported.
	 */
	private boolean checkAttributeTypes(
			...
			for (Graph graph : lhsAndRhsGraph) {
				for (Node node : graph.getNodes()) {
					for (Attribute attribute : node.getAttributes()) {
						if (!HenshinAGGExporter
								.isSupportedPrimitiveType(attribute.getType()
										.getEType())) {
							throw new UnsupportedRuleException(rule,
									attribute.getType());
						}
						// handling havaScript expressions
						Parameter param = rule.getParameter(attribute
								.getValue());
						if (attribute.getType().getEType() == EcorePackage.eINSTANCE
								.getEString()
								&& attribute.getConstant() == null
								&& param == null) {
	...
	/**
	 * Checks whether all of the passed rules have a common setting of their
	 * 'dangling option'.
	 * 
	 * @param rulesToBeChecked
	 *            The rules, which are checked regarding a common setting of the
	 *            'dangling option'.
	 * @return <code>true</code> if the dangling option of all provided rules
	 *         are the same.
	 * @throws UnsupportedRuleException
	 *             On inconsistent dangling options within the rule set.
	 */
	private boolean consistentDanglingOption(
			List<org.eclipse.emf.henshin.model.Rule> rulesToBeChecked)
			throws UnsupportedRuleException {
		...
		for (org.eclipse.emf.henshin.model.Rule rule : rulesToBeChecked) {
			if (checkDanglingConsistent) {
				...
				if (checkDanglingActivated && checkDanglingDeactivated) {
					...
					throw new UnsupportedRuleException(
							UnsupportedRuleException.inconsistentDanglingOptions);
				}
			}
			if (injectiveMatchingConsistent) {
				...
				if (checkInjectiveMatchingActivated
						&& checkInjectiveMatchingDeactivated) {
					injectiveMatchingConsistent = false;
					throw new UnsupportedRuleException(
							UnsupportedRuleException.inconsistentInjectiveMatchingOptions);
				}
			}
		}
