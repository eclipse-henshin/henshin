...
public abstract class AbstractProducer<T> implements IProducer<T> {
	...
	protected Boolean getOption(String name) throws RuntimeException {
		...
		if (result == null) {
			RuntimeException e = new UnknownOptionException("Producer " + this.getName() + " does not register an option '" + name + "'");
			PAFActivator.getDefault().getLog().log(new Status(IStatus.ERROR, PAFActivator.PLUGIN_ID, "Configuration option '" + name + "' does not exist for producer '" + this.getName() + "'.", e)); //$NON-NLS-1$
			throw e;
		} else {
			return result;
		}		
