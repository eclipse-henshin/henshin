...
public class HenshinMappingUtil {
	...
	/**
	 * Create a new mapping between the origin and image node. If there exists
	 * already a mapping between the two nodes, it is returned instead.
	 * 
	 * @param origin
	 *            Origin node.
	 * @param image
	...
	/**
	 * Get the mappings used for a given rule graph. The graph must be contained
	 * directly or indirectly in a {@link Rule} and must be either the Rhs or
	 * contained in a {@link NestedCondition}.
	 * 
	 * @param imageGraph
	 *            Image graph. Cannot be the Lhs.
	 * @return The mappings.
	 */
	...
	public static Mapping getNodeImageMapping(Node origin, Graph targetGraph, List<Mapping> mappings) {
		for (Mapping mapping : mappings) {
			if (mapping.getOrigin() == origin && mapping.getImage().getGraph() == targetGraph)
				return mapping;
		}
		return null;
	}
	...
	public static Attribute getAttributeImage(Attribute attribute, Graph targetGraph,
		...
		if (attribute.getNode() == null) return null;
		Node nodeImage = getNodeImage(attribute.getNode(), targetGraph, mappings);
		if (nodeImage == null) return null;
	...
	public static Attribute getAttributeOrigin(Attribute attribute, List<Mapping> mappings) {
		...
		if (nodeOrigin == null) return null;
	...
	/**
	 * Find the image of a node or an edge. This is a wrapper for
	 * {@link #getNodeOrigin(Node, List)} and {@link #getEdgeOrigin(Edge, List)}
	 * .
	 * 
	 * @param <T>
	 *            Either {@link Node} or {@link Edge} class.
