...
public class GiraphRuleTemplate
{
  ...
  protected final String TEXT_46 = NL + "    // Next nodes are activated by a message, so we can put this one to sleep:" + NL + "    vertex.voteToHalt();" + NL + "" + NL + "  }" + NL + "" + NL + "  /**" + NL + "   * Pretty-print a match." + NL + "   * @param match The Match to be printed." + NL + "   * @return The printed string." + NL + "   */" + NL + "  private static String matchToString(BytesWritable match) {" + NL + "    if (match == null) {" + NL + "      return \"empty\";" + NL + "    }" + NL + "    byte[] bytes = match.getBytes();" + NL + "    StringBuffer r = new StringBuffer();" + NL + "    for (int e = 0; e < bytes.length; e++) {" + NL + "      r.append(bytes[e] + \" \");" + NL + "    }" + NL + "    return r.toString();" + NL + "  }" + NL + "" + NL + "  /**" + NL + "   * Get the vertex ID of a matched node." + NL + "   * @param match The match oject." + NL + "   * @param vertexIndex Index of the next vertex." + NL + "   * @return The vertex ID." + NL + "   */" + NL + "  private static BytesWritable getMatchVertexId(BytesWritable match," + NL + "    int vertexIndex) {" + NL + "    byte[] bytes = match.getBytes();" + NL + "    int d = 0;" + NL + "    for (int i = 0; i < vertexIndex; i++) {" + NL + "      d += bytes[d] + 1;" + NL + "    }" + NL + "    return new BytesWritable(Arrays.copyOfRange(bytes, d + 1, bytes[d]));" + NL + "  }" + NL + "" + NL + "  /**" + NL + "   * Extend a partial match." + NL + "   * @param match The match object." + NL + "   * @param vertexId The ID of the next matched vertex." + NL + "   * @return The extended match object." + NL + "   */" + NL + "  private BytesWritable addMatchVertex(BytesWritable match," + NL + "    BytesWritable vertexId) {" + NL + "    byte[] bytes = (match != null) ? match.getBytes() : EMPTY_BYTE_ARRAY;" + NL + "    byte[] id = vertexId.getBytes();" + NL + "    int superstep = (int) getSuperstep();" + NL + "    int d = 0;" + NL + "    for (int i = 0; i < superstep; i++) {" + NL + "      d += bytes[d] + 1;" + NL + "    }" + NL + "    byte[] result = Arrays.copyOf(bytes, d + 1 + id.length);" + NL + "    result[d] = (byte) id.length;" + NL + "    System.arraycopy(id, 0, result, d + 1, id.length);" + NL + "    BytesWritable theResult = new BytesWritable(result);" + NL + "    LOG.info(\"Extending match: \" + matchToString(match) +" + NL + "      \" -> \" + matchToString(theResult));" + NL + "    return theResult;" + NL + "  }" + NL + "" + NL + "  /**" + NL + "   * Apply the rule to a given match." + NL + "   * @param vertex The base vertex." + NL + "   * @param match The match object." + NL + "   * @throws IOException On I/O errors." + NL + "   */" + NL + "  private void applyRule(Vertex<BytesWritable, IntWritable," + NL + "    IntWritable> vertex, BytesWritable match) throws IOException {" + NL + "" + NL + "    LOG.info(\"Applying rule ";
